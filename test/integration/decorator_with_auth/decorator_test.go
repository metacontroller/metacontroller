...
// Package decorator_with_auth contains integration tests for webhook
// authentication and connection configuration in DecoratorControllers.
package decorator_with_auth
...
func TestBearerTokenAuth(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			ns := "test-dec-bearer-" + strings.ToLower(tc.name)
			parentName := ns
			...
			parentCRD, parentClient := f.CreateCRD("TestDecBearerParent"+tc.name, apiextensions.NamespaceScoped)
			childCRD, childClient := f.CreateCRD("TestDecBearerChild"+tc.name, apiextensions.NamespaceScoped)
			...
			srv, caPEM := f.ServeWebhookTLSWithBearerAuth(testBearerToken, func(body []byte) ([]byte, error) {
				...
				resp := v1.DecoratorHookResponse{Attachments: []*unstructured.Unstructured{child}}
			...
			f.CreateDecoratorControllerWithBearerAuth(
				parentName,
				srv.URL+"/sync",
				caPEM,
...
func TestBasicAuth(t *testing.T) {
	...
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			ns := "test-dec-basic-" + strings.ToLower(tc.name)
			parentName := ns
			labels := map[string]string{"test": ns}
			...
			f.CreateNamespace(ns)
			parentCRD, parentClient := f.CreateCRD("TestDecBasicParent"+tc.name, apiextensions.NamespaceScoped)
			childCRD, childClient := f.CreateCRD("TestDecBasicChild"+tc.name, apiextensions.NamespaceScoped)
			...
			f.CreateDecoratorControllerWithBasicAuth(
				parentName,
				srv.URL+"/sync",
				caPEM,
...
func TestEndpointConfigsBearerToken(t *testing.T) {
	...
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			ns := "test-dec-conn-bearer-" + strings.ToLower(tc.name)
			parentName := ns
			labels := map[string]string{"test": ns}
			...
			f.CreateNamespace(ns)
			parentCRD, parentClient := f.CreateCRD("TestDecConnParent"+tc.name, apiextensions.NamespaceScoped)
			childCRD, childClient := f.CreateCRD("TestDecConnChild"+tc.name, apiextensions.NamespaceScoped)
			...
			f.CreateDecoratorControllerWithEndpointConfigs(
				parentName,
				srv.URL+"/sync",
				endpointConfigs,
				framework.CRDResourceRule(parentCRD),
