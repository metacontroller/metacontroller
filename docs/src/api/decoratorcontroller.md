...
associative array that maps from the attachment's path to the JSON representation, 
like what you might get from `kubectl get <attachment-resource> <attachment-name> -o json`.
...
If the attachment resource is namespaced, the key will always be of the form 
`{.metadata.namespace}/{.metadata.name}`. If the attachment resource is 
cluster scoped, the key is only the object's `.metadata.name`.
